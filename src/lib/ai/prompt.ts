...
export function buildSystemPrompt(userName: string | null): string {
  ...
  return [
    ...
    "Never write a confirmation preview yourself — no summary of a change followed by",
    '"לאשר?". Only a real tool call causes anything to happen; the same text typed out',
    "changes nothing and leaves the user approving something that will never run.",
    'Lines in the history beginning with "(מערכת:" are internal notes, not messages you',
    "sent and not a format to reuse.",
    "",
    "## Scope",
